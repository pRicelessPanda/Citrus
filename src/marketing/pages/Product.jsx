import { useNavigate } from 'react-router-dom';
import {
  Languages, ShieldCheck, Search, GitCompare, FolderKanban, PenLine,
  ArrowRight, Check, Sparkles,
} from 'lucide-react';
import { Reveal, Eyebrow, GradientText, GlowBlob, MkButton } from '../mkui.jsx';
import InteractiveDemo from '../InteractiveDemo.jsx';
import { CTASection } from './Home.jsx';

const SECTIONS = [
  {
    icon: Languages,
    eyebrow: 'Understand',
    title: 'Every paper, in plain language',
    body: 'Paste a link or upload a PDF and Citrus returns a sectioned explainer — Background, Methodology, Results, and more — written at the reading level you choose. Highlight any sentence to ask a question with full paper context.',
    points: ['Four reading levels, per section', 'Highlight-to-ask with source citations', 'One-click APA / MLA / Chicago / BibTeX export'],
    mock: <MockTranslate />,
  },
  {
    icon: ShieldCheck,
    eyebrow: 'Evaluate',
    title: 'Credibility you can defend',
    body: 'Every source gets a transparent 0–10 score built from currency, journal quality, author credentials, and accuracy — each expandable so you always know why. Inside projects, a fifth Relevance subscore ranks sources against your hypothesis.',
    points: ['Predatory-journal & retraction checks', 'Evidence-strength rating (1–4)', 'Relevance scoring inside projects'],
    mock: <MockScore />,
    flip: true,
  },
  {
    icon: GitCompare,
    eyebrow: 'Compare',
    title: 'Two papers, side by side',
    body: 'Compare any two papers section-by-section on their raw academic text. Agreement is highlighted green, disagreement red, and a similarity score summarizes the overlap — with an AI chat scoped to both papers.',
    points: ['Auto-identified sections', 'Agreement highlighting', 'Side-by-side credibility'],
    mock: <MockCompare />,
  },
  {
    icon: FolderKanban,
    eyebrow: 'Organize',
    title: 'Run your whole research program',
    body: 'Projects are AI-assisted workspaces: generate a project overview, get credibility-ranked suggested papers, keep references with timestamps, and chat with an assistant that has full context of everything inside.',
    points: ['AI idea generation & literature review', 'Credibility-ranked suggestions', 'Shared workspace with collaborators'],
    mock: <MockProject />,
    flip: true,
  },
  {
    icon: PenLine,
    eyebrow: 'Write',
    title: 'Draft papers from your sources',
    body: 'Select sources per section and Citrus writes each one with automatic in-text citations, clickable references, and flags on any sentence that needs a citation. Collaborate in real time with regular and AI channels.',
    points: ['Automatic in-text citations', 'Citation-needed flagging', 'APA / IEEE / Nature writing styles'],
    mock: <MockWrite />,
  },
];

export default function Product() {
  const navigate = useNavigate();
  return (
    <>
      <section className="relative overflow-hidden pt-36 pb-16 text-center">
        <GlowBlob className="-top-20 left-1/2 -translate-x-1/2" size={520} opacity={0.18} />
        <div className="relative mx-auto max-w-4xl px-6">
          <Reveal><Eyebrow><Sparkles size={13} /> The product</Eyebrow></Reveal>
          <Reveal delay={60}>
            <h1 className="mt-5 font-display text-5xl leading-tight md:text-7xl">
              One workspace for <GradientText>the entire</GradientText> research lifecycle
            </h1>
          </Reveal>
          <Reveal delay={120}>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-white/65">
              Understand, evaluate, compare, organize, and write — without leaving Citrus.
            </p>
          </Reveal>
          <Reveal delay={180}>
            <div className="mt-8 flex justify-center gap-3">
              <MkButton size="lg" onClick={() => navigate('/dashboard')}>Launch Citrus <ArrowRight size={18} /></MkButton>
            </div>
          </Reveal>
        </div>
      </section>

      <section className="px-6 pb-8">
        <Reveal><InteractiveDemo /></Reveal>
      </section>

      {/* deep-dive alternating sections */}
      <div className="mx-auto max-w-7xl px-6 py-16">
        {SECTIONS.map((s, i) => (
          <div key={s.title} className={`grid items-center gap-12 py-16 md:grid-cols-2 ${s.flip ? 'md:[direction:rtl]' : ''}`}>
            <Reveal className="md:[direction:ltr]">
              <Eyebrow><s.icon size={13} /> {s.eyebrow}</Eyebrow>
              <h2 className="mt-4 font-display text-4xl leading-tight">{s.title}</h2>
              <p className="mt-4 text-white/65">{s.body}</p>
              <ul className="mt-6 space-y-3">
                {s.points.map((p) => (
                  <li key={p} className="flex items-center gap-3 text-white/80">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-lime/20 text-lime"><Check size={13} /></span>
                    {p}
                  </li>
                ))}
              </ul>
            </Reveal>
            <Reveal delay={100} className="md:[direction:ltr]">
              <div className="relative">
                <GlowBlob className={`${s.flip ? '-left-10' : '-right-10'} top-1/2 -translate-y-1/2`} size={280} opacity={0.12} />
                <div className="relative rounded-3xl border border-white/10 bg-white/[0.03] p-6 shadow-2xl">{s.mock}</div>
              </div>
            </Reveal>
          </div>
        ))}
      </div>

      <CTASection />
    </>
  );
}

/* ---- lightweight mocks ---- */
function MockTranslate() {
  return (
    <div>
      <div className="text-base font-medium">Attention Is All You Need</div>
      <div className="mt-1 text-xs text-white/50">Vaswani et al. · 2017</div>
      {['Background', 'Methodology'].map((s) => (
        <div key={s} className="mt-3 rounded-xl border border-white/10 bg-white/[0.02] p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="font-display">{s}</span>
            <span className="rounded bg-lime/15 px-2 py-0.5 text-[10px] text-lime">Curious Adult</span>
          </div>
          <div className="space-y-1.5"><div className="h-2 rounded bg-white/10" /><div className="h-2 w-4/5 rounded bg-white/10" /></div>
        </div>
      ))}
    </div>
  );
}
function MockScore() {
  return (
    <div className="grid grid-cols-[100px_1fr] gap-4">
      <div className="flex flex-col items-center justify-center rounded-xl bg-lime/10 p-3">
        <span className="font-display text-4xl text-lime">9.5</span><span className="text-xs text-white/40">/10</span>
      </div>
      <div className="space-y-2.5">
        {[['Currency', 94], ['Journal', 98], ['Authors', 96], ['Accuracy', 92]].map(([l, w]) => (
          <div key={l}>
            <div className="mb-1 flex justify-between text-xs text-white/60"><span>{l}</span></div>
            <div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-lime" style={{ width: `${w}%` }} /></div>
          </div>
        ))}
      </div>
    </div>
  );
}
function MockCompare() {
  const tones = [
    'border-success/30 bg-success/15',
    'border-warning/30 bg-warning/15',
    'border-danger/30 bg-danger/15',
  ];
  return (
    <div className="space-y-2">
      {tones.map((tone, i) => (
        <div key={i} className="grid grid-cols-2 gap-2">
          <div className={`rounded-lg border p-2.5 ${tone}`}><div className="h-2 w-4/5 rounded bg-white/15" /></div>
          <div className={`rounded-lg border p-2.5 ${tone}`}><div className="h-2 w-3/4 rounded bg-white/15" /></div>
        </div>
      ))}
    </div>
  );
}
function MockProject() {
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 rounded-lg bg-white/5 p-2.5"><span className="h-3 w-3 rounded-full bg-lime" /><span className="text-sm">Attention in low-resource NLP</span></div>
      {['Attention Is All You Need · 9.5', 'Cross-lingual Transfer · 8.1'].map((r) => (
        <div key={r} className="rounded-lg border border-white/10 bg-white/[0.02] p-2.5 text-sm text-info">{r}</div>
      ))}
    </div>
  );
}
function MockWrite() {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
      <div className="font-display text-lg">Introduction</div>
      <p className="mt-2 text-sm leading-relaxed text-white/70">
        Transformer models have reshaped NLP <span className="text-info">[1]</span>. Recent work extends them to low-resource settings <span className="text-info">[2]</span>,
        <span className="rounded bg-warning/20 px-1 text-warning"> though prevalence figures remain uncited.</span>
      </p>
    </div>
  );
}
